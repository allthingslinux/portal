...
- **Viewing/editing users**: Query keys in `@portal/api/query-keys` under `users.*`
- **Ban management**: Uses `banExpires` date field — validated via Zod schema in `@portal/schemas/user`
...
- **API Keys**: The `apikey` table uses `referenceId` (not `userId`) as of BetterAuth v1.5 — queries filter by `apikey.referenceId`
...
- **Never expose raw DB IDs** in admin UI responses — use DTOs from `@portal/types/common`.
