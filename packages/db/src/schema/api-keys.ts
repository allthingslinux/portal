...
export const apikey = pgTable(
  "apikey",
  {
    ...
    configId: text("config_id").notNull().default("default"), // The config ID for multi-config support (v1.5+)
    ...
    referenceId: text("reference_id")
      .notNull()
      .references(() => user.id, { onDelete: "cascade" }), // The ID of the entity that owns this key (userId or organizationId)
    ...
    // Rate limit defaults reference RATE_LIMIT constants from @portal/utils/constants
