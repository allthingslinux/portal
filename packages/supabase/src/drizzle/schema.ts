...
import { pgTable, index, uniqueIndex, foreignKey, unique, pgPolicy, check, uuid, varchar, text, boolean, timestamp, jsonb, serial, integer, inet, bigint, primaryKey, pgView, pgEnum, pgSchema } from "drizzle-orm/pg-core"
...
export const appPermissions = pgEnum("app_permissions", ['roles.manage', 'settings.manage', 'members.manage', 'invites.manage'])
...
export const accounts = pgTable("accounts", {
	...
	pgPolicy("accounts_read", { as: "permissive", for: "select", to: ["authenticated"], using: sql`((( SELECT auth.uid() AS uid) = primary_owner_user_id) OR has_role_on_account(id) OR is_account_team_member(id))` }),
	...
	pgPolicy("delete_team_account", { as: "permissive", for: "delete", to: ["authenticated"] }),
...
export const invitations = pgTable("invitations", {
	...
	pgPolicy("super_admins_access_invitations", { as: "permissive", for: "select", to: ["authenticated"], using: sql`is_super_admin()` }),
	...
	pgPolicy("invitations_read_self", { as: "permissive", for: "select", to: ["authenticated"] }),
...
export const nonces = pgTable("nonces", {
	...
	index("idx_nonces_status").using("btree", table.clientToken.asc().nullsLast().op("text_ops"), table.userId.asc().nullsLast().op("text_ops"), table.purpose.asc().nullsLast().op("text_ops"), table.expiresAt.asc().nullsLast().op("text_ops")).where(sql`((used_at IS NULL) AND (revoked = false))`),
	index("idx_nonces_verify_lookup").using("btree", table.purpose.asc().nullsLast().op("text_ops"), table.expiresAt.desc().nullsFirst().op("text_ops"), table.userId.asc().nullsLast().op("text_ops")).where(sql`((used_at IS NULL) AND (revoked = false))`),
	foreignKey({
...
export const notifications = pgTable("notifications", {
	...
	index("idx_notifications_account_dismissed").using("btree", table.accountId.asc().nullsLast().op("timestamptz_ops"), table.dismissed.asc().nullsLast().op("timestamptz_ops"), table.expiresAt.asc().nullsLast().op("timestamptz_ops")),
	foreignKey({
			columns: [table.accountId],
			foreignColumns: [accounts.id],
			name: "notifications_account_id_fkey"
		}).onDelete("cascade"),
	pgPolicy("notifications_read_self", { as: "permissive", for: "select", to: ["authenticated"], using: sql`((account_id = ( SELECT auth.uid() AS uid)) OR has_role_on_account(account_id))` }),
	pgPolicy("notifications_update_self", { as: "permissive", for: "update", to: ["authenticated"] }),
	pgPolicy("restrict_mfa_notifications", { as: "restrictive", for: "all", to: ["authenticated"] }),
]);
...
export const rolePermissions = pgTable("role_permissions", {
	...
	pgPolicy("restrict_mfa_role_permissions", { as: "restrictive", for: "all", to: ["authenticated"], using: sql`is_mfa_compliant()` }),
...
export const accountsMemberships = pgTable("accounts_memberships", {
	...
	pgPolicy("accounts_memberships_delete", { as: "permissive", for: "delete", to: ["authenticated"], using: sql`((user_id = ( SELECT auth.uid() AS uid)) OR can_action_account_member(account_id, user_id))` }),
	pgPolicy("accounts_memberships_read", { as: "permissive", for: "select", to: ["authenticated"] }),
	pgPolicy("restrict_mfa_accounts_memberships", { as: "restrictive", for: "all", to: ["authenticated"] }),
...
export const userAccountWorkspace = pgView("user_account_workspace", {	id: uuid(),
	...
	pictureUrl: varchar("picture_url", { length: 1000 }),
}).as(sql`SELECT id, name, picture_url FROM accounts WHERE primary_owner_user_id = (( SELECT auth.uid() AS uid)) AND is_personal_account = true LIMIT 1`);
