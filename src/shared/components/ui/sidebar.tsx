...
> = ({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((next: boolean) => boolean)) => {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: stores sidebar preference client-side
  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      <div
        className={cn(
          "group/sidebar flex min-h-svh w-full text-sidebar-foreground has-data-[variant=inset]:bg-sidebar",
          className
        )}
        data-minimized={!open}
        ref={ref}
...
> = ({ className, asChild = false, ...props }) => {
  ...
  return (
    <Comp
      className={cn(
        "flex h-8 shrink-0 items-center rounded-md px-2 font-medium text-muted-foreground text-xs outline-hidden ring-sidebar-ring transition-[margin,opa] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
          ...
          return (
            <Container key={`collapsible-${item.label ?? index}`}>
              <SidebarGroup key={item.label}>
                ...
                >
                  <SidebarGroupLabel asChild className={cn({ hidden: !open })}>
                    <CollapsibleTrigger>
                      <Trans defaults={item.label} i18nKey={item.label} />
                  ...
                  <SidebarMenu>
                    <ContentContainer>
                      {item.children.map((child, childIndex) => {
                              ...
                              <CollapsibleTrigger asChild>
                                <SidebarMenuButton tooltip={child.label}>
                                  ...
                                  >
                                    ...
                                    <span
                                      className={cn(
                                        "w-auto transition-opacity transition-width duration-500",
                                        {
                                          "w-0 opacity-0": !open,
                                        }
                                      )}
                        ...
                        const childKey =
                          "path" in child ? child.path : `child-${childIndex}`;
                        return (
                          <ChildContainer key={`group-${index}-${childKey}`}>
                            <SidebarMenuItem>
                              <TriggerItem />
                                            ...
                                            >
                                              ...
                                              >
                                                ...
                                                <span className={spanClassName}>
                                                  <Trans
                                                    defaults={subChild.label}
                                                    i18nKey={subChild.label}
                                                  />
                                                </span>
                                              </Link>
                                            </SidebarMenuSubButton>
